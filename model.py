...
class DQN(nn.Module):
    def __init__(self, h, w, outputs):
        ...
        convw = conv2d_size_out(conv2d_size_out(
            conv2d_size_out(conv2d_size_out(w))))
        convh = conv2d_size_out(conv2d_size_out(
            conv2d_size_out(conv2d_size_out(h))))
...
def torch_to_keras(model, image_shape):
    ...
    # add batch dimension
    input_np = np.random.uniform(0, 1, [1, *image_shape])
    input_var = torch.autograd.Variable(torch.tensor(
        input_np, dtype=torch.float, device=device))
...
class DQNUser(nn.Module):
    def __init__(self, h, w, outputs, ksize, n_layers):
        ...
        convw = reduce(lambda convw, _: conv2d_size_out(
            convw), range(n_layers), w)
        convh = reduce(lambda convh, _: conv2d_size_out(
            convh), range(n_layers), h)
